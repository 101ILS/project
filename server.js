...
// Установите ваш API-ключ Google Gemini
...
app.post('/generate-content', async (req, res) => {
    ...
    if (!question) {
        return res.status(400).json({ status: 'error', error_code: 400, message: 'Вопрос не может быть пустым.' });
    }
    ...
    try {
        ...
        if (response.status === 200) {
            ...
            return res.status(500).json({ status: 'error', message: 'Ошибка при обращении к API Google', details: response.data });
        }
    } catch (error) {
